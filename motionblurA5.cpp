...
void *renderRows(void * args) {
	...
	for (uint y = startingRow; y < startingRow + rowsToRender; ++y) {
		...
		for (uint x = 0; x < nx; ++x) {
			...
			if(x < nx/2 && y < ny/2){
				// MOTION BLUR STUFF///////////////////////////////////////////////////////
				glm::vec3 motionColour(0,0,0);
				for(int t = 0; t < MOTIONBLUR; t++){
					std::random_device generator;
					// std::normal_distribution<double> distribution(0.5,0.125);
					std::uniform_real_distribution<double> distribution(0.0,1.0);
					// Equation t = t0 + random * (t1-t0)
					// But i'm going to assume t0 = 0 and t1 = 1 ayyyyyyyyyyy
					glm::vec3 newEye = glm::vec3(eye.x + distribution(generator)*3.0 ,eye.y,eye.z);
					glm::vec4 worldCoord = calcPinWorld(x,y,transformDeviceToWorld);
					Ray r = Ray(glm::vec4(newEye, 1), worldCoord-glm::vec4(newEye,1));
					r.time = distribution(generator);
					ColourHit ch =  rayColour(r, lights, root, ambient, 0);
					if(ch.hit){
						motionColour += ch.colour/(float)MOTIONBLUR;
						// glm::vec3 colour = ch.colour;

					} else{
						// background
						// float height = (float)y/ny;
						// motionColour.r += std::max(height-0.1,0.0) / (float) MOTIONBLUR;
						// motionColour.g += height*height / (float) MOTIONBLUR;
						// motionColour.b += std::max(0.176*(height)+0.25*(height)*(height),0.0)/(float)MOTIONBLUR;
						motionColour.r += 0.51 / (float) MOTIONBLUR;
						motionColour.g += 0.746 / (float) MOTIONBLUR;
						motionColour.b += 0.785 / (float) MOTIONBLUR;
					}
				}
				(*img)(x,y,0) = motionColour.r;
				(*img)(x,y,1) = motionColour.g;
				(*img)(x,y,2) = motionColour.b;
			} else {
				// NORMAL STUFF
				ColourHit ch =  rayColour(r, lights, root, ambient, 0);
				if(ch.hit){
			...
			// // std::cout << r.dir.z << std::endl;
			// double t = (-8- eye.z)/r.dir.z;
			// glm::vec4 fixedFocalPoint = r.dir * (float) t  + glm::vec4(eye,1);
			// std::cout << fixedFocalPoint .x << " " << fixedFocalPoint.y << " " << fixedFocalPoint.z << std::endl;
			// // std::cout << t << std::endl;
	...
	for (uint y = startingRow; y < startingRow + rowsToRender; ++y) {
		...
		for (uint x = 0; x < nx; ++x) {
			if(pixelDiff[y][x] > 0.15){
				...
				for(int i=-1; i<2; i++){
					for(int j=-1; j<2; j++){
						// skip it, since we already have this value from before
						if(x < nx/2 && y < ny/2){
							glm::vec3 motionColour(0,0,0);
							for(int t = 0; t < MOTIONBLUR; t++){
								std::random_device generator;
								...
								glm::vec3 newEye = glm::vec3(eye.x + distribution(generator)*3 ,eye.y,eye.z);
